...
class AllArt extends Component {
  ...
  buyItem = async () => {
    const res = await this.props.contract.methods
      .buyToken(this.props.art.tokenId)
      .send({
      ...
      >
        ...
        >
          ...
          <CardBody className='all-art-body'>
            ...
            <Col sm={{ size: 12 }}>
              {but === 'visible' ? (
                <Link to={`/card/${this.props.art.tokenId}`}>
                  <Button
                    className={but}
                    id='buy-bid-btn'
                    size='sm'
...
class AllItemComponent extends Component {
  ...
  async componentDidMount() {
    ...
    try {
      res = await Axios.get(
        `http://geo.superworldapp.com/api/json/tokens/get?networkId=4`
      );
    } catch (err) {
      console.error(err);
    }
  ...
  render() {
    const menu = this.state.art.map((x) => {
      return (
        <div key={x.tokenId} className='col-4 col-md-3' id='all-art-card'>
          <AllArt
            art={x}
